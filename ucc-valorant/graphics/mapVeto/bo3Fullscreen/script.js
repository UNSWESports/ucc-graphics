...
nodecg.listenFor('inflate', () => {
  expandMapBan(activeMap);
});

nodecg.listenFor('deflate', () => {
  deflateMapBan(eMapBan3Div);
  deflateMapBan(eMapBan4Div);
...
const expandMapBan = (mapBanDiv) => {
  ...
  setTimeout(() => {
    ...
    mapBanDiv.getElementsByClassName('mapText')[0].style.fontSize = '1.8em';
    mapBanDiv.getElementsByClassName('mapSubtext')[0].style.fontSize = '1.15em';
    ...
    mapBanDiv.style.zIndex = '1000';

    const expansionNode = document.createElement('div');
    expansionNode.style.width = `${width + divLMargin + divRMargin}px`;
    expansionNode.classList.add('expansionCompensation');
    mapBanDiv.parentElement.insertBefore(expansionNode, mapBanDiv);
...
const deflateMapBan = (mapBanDiv) => {
  ...
  setTimeout(() => {
    ...
    mapBanDiv.style.zIndex = '';

    for (let e of document.getElementsByClassName('expansionCompensation')) {
      e.remove();
    }
  }, 1500)
}